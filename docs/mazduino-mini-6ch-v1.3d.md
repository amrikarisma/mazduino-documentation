# Mazduino Mini 6CH (v1.3D)

## Gambaran Umum

Mazduino Mini 6CH v1.3D adalah **versi produksi terakhir** dari lini Mini 6CH. Secara fungsi board ini sama persis dengan v1.3C — yang berubah adalah rangkaian di sekitar bagian analog dan CAN. Dibangun di sekitar prosesor **ARM Cortex-M4 180 MHz** 32-bit yang powerful, memberikan manajemen mesin komprehensif dalam faktor bentuk kompak yang cocok untuk mesin 6-silinder atau mesin 4-silinder dengan output arus tinggi tambahan.
...
- **JP9 Tanpa Bridge Pabrik**: Jumper Knock/PC13 memakai footprint solder jumper **terbuka** — pada v1.3C sisi Knock sudah tersambung dari pabrik, di v1.3D Anda yang menentukan

!!! info "Pinout tidak berubah"
    Pinout konektor 48-pin dan pemetaan pin MCU **identik dengan v1.3C** (diverifikasi pin per pin dari file desain). Tune dan konfigurasi firmware v1.3C dapat dipakai langsung tanpa perubahan.

**Perbaikan yang diwarisi dari v1.3C (relatif terhadap v1.3B):**
...
- **Knock Input**: Satu input sensor knock yang dioptimalkan (PA3)
...
Mazduino Mini 6CH v1.3D memiliki 6 channel ignition dan 6 channel injection yang seluruhnya dapat dikonfigurasi sesuai kebutuhan aplikasi.
...
Mazduino Mini 6CH v1.3D dilengkapi dengan sistem jumper solder yang memungkinkan kustomisasi fungsi sesuai kebutuhan aplikasi Anda. Jumper ini memberikan fleksibilitas konfigurasi tanpa perlu mengubah firmware, memastikan kompatibilitas optimal dengan berbagai setup mesin.
...
#### Ignition VDrive 5-6 Jumper *(sejak v1.3C)*
...
- **Mode Knock**: Input sensor knock untuk deteksi ketukan mesin

- **Mode PC13**: Akses langsung ke pin MCU untuk fungsi custom

- **Pin MCU**: Satu input knock yang dioptimalkan pada PA3

- **Berubah di v1.3D**: JP9 memakai footprint solder jumper **terbuka**. Pada v1.3C sisi Knock sudah terhubung dari pabrik; di v1.3D pin 8 tidak tersambung ke mana pun sampai Anda menjembatani salah satu sisi JP9. **Jika sensor knock tidak terbaca sama sekali pada board baru, periksa JP9 lebih dulu.**
...
Mazduino Mini 6CH v1.3D menggunakan konektor ECU Yamaha 48-pin dengan pin assignment sebagai berikut:
...
| 8 | **Knock/PC13** | Input sensor knock atau akses pin MCU langsung — pilih lewat JP9, yang di v1.3D datang terbuka |
...
### Pin Mapping MCU (v1.3D)

Pin assignment **STM32F427VGT6** untuk v1.3D. **Tidak ada satu pun pin yang berpindah dari v1.3C** — kolom terakhir mencatat perubahan yang terjadi pada v1.3C relatif terhadap v1.3B, dan dipertahankan di sini untuk pengguna yang datang dari board lama.
...
#### Knock Input (sejak v1.3C)
- **Pin 8 (Knock/PC13)**: Input sensor knock, dipilih lewat solder jumper JP9 — di v1.3D jumper ini terbuka dari pabrik, jadi harus dijembatani lebih dulu
...
#### Ignition VDrive Terpisah (sejak v1.3C)
...
- **Kapan ini benar**: ECU berada di **ujung** bus CAN. Bus CAN yang sehat punya tepat **dua** terminator 120 Ω, satu di tiap ujung

- **PERHATIAN**: Jika ECU dipasang di **tengah** bus yang kedua ujungnya sudah diterminasi, total resistansi turun ke 40 Ω dan bus bisa gagal berkomunikasi. Pada kasus itu **lepas salah satu terminator lain**, atau angkat R13 dari board

- **Perangkat Mazduino lain**: Racedash dan WBO CAN Controller juga bisa membawa terminatornya sendiri — hitung total terminator di seluruh jaringan, bukan per perangkat
...
5. **Knock Sensor**: Jembatani JP9 ke sisi Knock lebih dulu (v1.3D datang terbuka), lalu hubungkan sensor knock ke pin 8
...
### Catatan Wiring v1.3D
- **Terminasi CAN**: R13 (120 Ω) terpasang tetap di board. Pastikan seluruh bus CAN tetap punya tepat dua terminator — lihat [Terminasi CAN Onboard](#terminasi-can-onboard-baru-di-v13d)

- **JP9 Terbuka**: Pin 8 tidak tersambung ke jalur Knock maupun PC13 sampai JP9 dijembatani
...
Mazduino Mini 6CH v1.3D didesain dengan **ARM Cortex-M4 180 MHz** sebagai MCU utama, memberikan performa tinggi dengan clock 180MHz. Mendukung beberapa module tambahan seperti VR Conditioner Module dan Idle Stepper Module. Secara bawaan Mazduino Mini 6CH v1.3D hanya mendukung sinyal CKP dan CMP dalam bentuk digital, seperti sensor hall dan optical. Untuk kendaraan yang masih menggunakan VR sensor perlu menambahkan VR Module (atau bisa custom sensor ke hall).

![VR Module](img/mini6ch/vr-conditioner-module.jpeg)

Kemudian untuk bagian Idle, Mazduino Mini 6CH v1.3D sudah support Idle PWM, terdapat 2 output dari low side mosfet yang dapat digunakan untuk kontrol IDLE PWM.
...
- **Knock/MCU PC13 (JP9):** Opsi input Knock sensor atau langsung ke pin MCU PC13. **Di v1.3D jumper ini datang dalam keadaan terbuka** — tidak ada sisi yang tersambung dari pabrik, berbeda dengan v1.3C yang default-nya Knock
...
### Konfigurasi Khusus v1.3D

File konfigurasi v1.3C dapat dipakai apa adanya — pinout keduanya identik.
...
- **Pin Mapping**: Gunakan file konfigurasi pin mapping khusus v1.3D

### Firmware Support
- **rusEFI**: Dukungan penuh untuk v1.3D dengan pin mapping yang diperbarui

- **Speeduino**: Tidak didukung pada v1.3D — gunakan rusEFI
...
### Migrasi dari v1.3C
- **Firmware/Tune**: Tidak ada yang perlu diubah — pinout konektor dan pemetaan pin MCU identik

- **JP9**: Jembatani sisi Knock jika Anda memakai sensor knock; board tidak lagi datang dengan sisi itu tersambung

- **CAN**: Periksa jumlah terminator di jaringan sebelum menyambung — board ini membawa 120 Ω sendiri

- **Kalibrasi Analog**: Periksa ulang kalibrasi sensor analog; referensi ADC-nya kini presisi, bukan pembagi resistor
...
Rekomendasi ukuran kabel per pin untuk Mazduino Mini 6CH v1.3D, lengkap dengan rekap jumlah kabel tiap ukuran, tersedia di [Rekomendasi Ukuran Kabel Harness](rekomendasi-ukuran-kabel.md).
